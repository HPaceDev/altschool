...
export default async function QuestionsPage({
  ...
  const areas = [...new Set(all.map((q) => q.area))].sort((a, b) =>
    a.localeCompare(b, "ru"),
  );

  const visible = all
    .filter((q) =>
      filters.status ? q.status === filters.status : q.status !== "withdrawn",
    )
  ...
  const blockers = all.filter(
    (q) => q.status === "open" && q.priority === "blocker",
  ).length;
  ...
  return (
    <div className="mx-auto max-w-4xl px-5 py-8 sm:px-6 sm:py-10">
      <PageHeader
        ...
        lead={
          <>
            Здесь мы задаём всё, что нужно решить до разработки. Отвечайте прямо
            на странице вопроса: ответ сохранится с вашим именем и временем.
            Если позже вы измените мнение — просто напишите новый ответ, прежний
            останется в истории.
          </>
        }
        actions={
        ...
        <Card className="mb-6 border-important/30 bg-important-soft px-4 py-3">
          <p className="text-sm text-important">
            <strong className="font-semibold">
              Ждём ответа по{" "}
              {plural(waiting, "вопросу", "вопросам", "вопросам")}
            </strong>
            {blockers > 0
            ...
            <li key={q.id}>
              ...
              >
                ...
                {q.latestAnswer ? (
                  <p className="mt-1.5 line-clamp-2 text-sm text-ink-muted">
                    <span className="text-ink-faint">
                      {q.latestAnswer.authorName},{" "}
                      {formatDateTime(q.latestAnswer.createdAt)}:{" "}
                    </span>
                    {q.latestAnswer.body}
                  </p>
                ) : (
                ...
                <div className="mt-2 flex flex-wrap gap-3 text-xs text-ink-faint">
                  {q.answers.length > 1 ? (
                    <span>
                      {plural(q.answers.length, "версия", "версии", "версий")}{" "}
                      ответа
                    </span>
                  ) : null}
                  {q.commentCount > 0 ? (
                  ...
                  {q.approvalCount > 0 ? (
                    <span className="text-done">утверждено</span>
                  ) : null}
                </div>
              </Link>
            </li>
