...
/**
 * Условные координаты регионов на схеме карты.
 *
 * Это не география: настоящих координат у школ пока нет, а картографический
 * сервис не выбран (вопрос Q-025). Точки расставлены так, чтобы читался
 * запад — восток и было видно, где пусто. На экране это прямо подписано.
 */
export const REGION_POINT: Record<string, { x: number; y: number }> = {
  "Санкт-Петербург": { x: 12, y: 20 },
  Москва: { x: 17, y: 38 },
  "Краснодарский край": { x: 11, y: 76 },
  "Республика Татарстан": { x: 30, y: 50 },
  "Свердловская область": { x: 42, y: 30 },
  "Новосибирская область": { x: 57, y: 62 },
  "Красноярский край": { x: 70, y: 42 },
};
